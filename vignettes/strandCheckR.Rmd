...
sliding window. That can be used to quantify the amount of reads coming from 
...
as would be expected from contaminating DNA.
It can also be used to detect regions with specific behavior (eg highest 
coverage) through the whole genome and not only feature regions.
...
length & step etc.
In a typique example of a human RNA-seq bam file of more than 4GB, it takes
about 3 minutes to scan the whole file and return a DataFrame of least than 2 
Gb.

A function to filter out reads coming from double strand DNA is provided.
This function also uses a sliding window to pick out contaminated reads. 
...
Then each read is assigned to a probability of being kept or not, 
