...
List computeWinInfo(IntegerVector covLen,IntegerVector covVal,int end,int win,int step){
  ...
  std::vector<double> sumCoverage;
  std::vector<int> MaxC;
  int maxCovP=0;
  while (start<=end){
    int sum=increaseVal(covLen,covVal,iP,start,win,preP,maxCovP);
    increase(covLen,iP,start,end,step,preP);
    if (sum>0){
      Start.push_back(1+start/step);
      sumCoverage.push_back(sum);
      MaxC.push_back(maxCovP);
    }
    start+=step;
  }
  return DataFrame::create(_["Start"]=Start,_["SumCoverage"]=sumCoverage, _ ["MaxCoverage"]= MaxC);
}
