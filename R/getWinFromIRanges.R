...
#' @param limit A read is considered to be included in a window if at least \code{limit} of it is in the window. 
...
#'
#'
getWinFromIRanges <- function(x, winWidth = 1000L, winStep = 100L, limit = 0, maxWin = Inf){
  
  limit <- limit[1]
  if (limit < 0 || limit > 1) stop("limit must be within the range [0, 1]")
  
  stopifnot(is.numeric(maxWin))
  
  # Calculate the index of the first window that overlaps a fragment
  startWin <- ceiling((start(x) - winWidth + (limit)*width(x)) / winStep) + 1
  startWin[startWin < 1] <- 1
  startWin[startWin > maxWin] <- maxWin
  
  # Calculate the index of the final window that overlaps a fragment
  endWin <- floor((end(x) - 1 - (limit)*width(x)) / winStep) + 1
