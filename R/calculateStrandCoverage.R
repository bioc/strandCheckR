...
#' 
#' @param winPositiveAlignments a list that has a `Coverage` field containing 
#' coverage coming from positive reads
#' @param winNegativeAlignments a list that has a `Coverage` field containing 
...
calculateStrandCoverage <- function(winPositiveAlignments, 
                                    winNegativeAlignments, 
                                    winWidth = 1000, winStep= 100){
    # make sure winPositiveAlignments$Coverage and 
    # winNegativeAlignments$Coverage have the same length to avoid some 
    # warnings afterward
    ...
    if (lenNC < lastBase) {
        winNegativeAlignments$Coverage <- c(winNegativeAlignments$Coverage,
                                            rep(0, lastBase - lenNC))
    } 
    if (lenPC < lastBase){
    winPositiveAlignments$Coverage <- c(winPositiveAlignments$Coverage,
                                        rep(0, lastBase - lenPC))
    }
    ...
    maxCoverage <- Rle(max(Views(winPositiveAlignments$Coverage + 
                                winNegativeAlignments$Coverage, 
                                start = st, end=end)))
    list("CovPositive"=CovPositive,"CovNegative"=CovNegative,
