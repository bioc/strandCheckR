...
# #' 
# #' @param Win a data frame contains the information of every window in \code{sequences}
...
# #' 
getWinInSequence <- function(Win, sequences, sequenceInfo, winWidth = 1000, winStep = 100){
  
  # Check the correct columns are in the sequenceInfo df
  reqCols <- c("FirstBaseInPartition", "LastBaseInPartition")
  if (!all(reqCols %in% names(sequenceInfo))) stop("sequenceInfo must contain the columns ", reqCols)
  ...
  for (i in seq_along(sequences)){
    if (!is.na(sequenceInfo$FirstBaseInPartition[i])){
      ...
      idFirst <- ceiling(sequenceInfo$FirstBaseInPartition[id] / winStep) # id of the first window of the sequence
      idLast <- ceiling((sequenceInfo$LastBaseInPartition[id] - winWidth+1) / winStep)# id of the last window of the sequence
      idRows <- which(Win$Start >= idFirst & Win$Start <= idLast) #get the windows of the sequence
