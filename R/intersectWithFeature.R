...
#'
#' @description Intersect the windows with an annotation data frame to get features that overlap with each window
#'
...
#' It can have mcols which contains the information or features that could be able to 
#' integrate to the input windows 
#' @param getFeatureInfo whether to get the information of features in the mcols of annotation data or not. 
#' If FALSE the return windows will have an additional column indicating whether a window overlaps with 
#' any range of the annotion data.
#' If TRUE the return windows will contain the information of features that overlap each window
#' @param overlapCol the columnn name of the return windows indicating whether a window overlaps with 
#' any range of the annotion data.
#' @param mcolsAnnot the column names of the mcols of the annotation data that you want to get information
...
#' #just want to know whether there's any transcript that overlaps with each window
...
intersectWithFeature <- function(windows, annotation, getFeatureInfo = FALSE, overlapCol = "OverlapFeature", mcolsAnnot, collapse, ...){
  ...
  w <- GRanges(seqnames = windows$Seq, ranges = IRanges(start = windows$Start, end = windows$End))
  ...
  if (!missing(mcolsAnnot)){
    if (length(mcolsAnnot)>0){
      ...
      if (length(mcolsAnnot)==0){
        message("mcols of annotation does not contain any column ",paste0(mcolsAnnot,collapse = ","))
