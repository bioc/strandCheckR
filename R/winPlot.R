...
winPlot <- function(windows,winfile,xlim){
  ...
  for (p in c(0.6,0.7,0.8,0.9)){
    p1 <- seq(p,1,0.001)
    n1 = ((qnorm(0.95)/sqrt(p1*(1-p1))/(log(p1/(1-p1))-log(p/(1-p))))^2)
    lines(n1, p1, type = "l", ylim = c(0, 1),xlim = c(1,m), col = colors[i])
    lines(n1, 1-p1, type = "l", ylim = c(0, 1),xlim = c(1,m), col = colors[i])
